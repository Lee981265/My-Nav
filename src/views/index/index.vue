<template>
  <div ref="navs" class="view">
    ...
    <div class="auto">
      ...
      <div class="T1" v-for="(T1,K1) in novelty" :key="T1.id">
        <h1 class="pageT1"><v-svg :data="'#icon-'+ T1.T1Name"></v-svg>{{T1.pageT1}}</h1>
        <div class="T2" v-for="(T2,K2) in T1.pageC1" :key="T2.id">
          <h2 class="pageT2"><v-svg :data="'#icon-'+ T2.T2Name"></v-svg>{{T2.pageT2}}</h2>
          <div class="main">
            <div class="lists" :style="'order:' + T2.pageC2[K3].Num" @click="onCopy(K1,K2,K3,List)" v-for="(List,K3) in T2.pageC2" :key="List.id">
